 ...
 *
 * This file copied/borrowed from WebCalendar:
...
function die_miserable_death($error, $anchor = '')
{
  ...
  $appStr = (empty($APPLICATION_NAME) ? 'HomeCare' : $APPLICATION_NAME);
  $url = $TROUBLE_URL;
  if (!empty($anchor)) {
...
/**
 * Get the full path to a file located in the homecare includes directory.
 */
 ...
 * (when HOMECARE_USE_ENV is set to true) or the settings.php file. It sets up
...
function do_config($callingFromInstall=false)
{
  global $db_database, $db_debug, $db_host, $db_login, $db_password, $db_persistent,
    $db_type, $phpdbiVerbose, $PROGRAM_DATE,
  ...
  $PROGRAM_DATE = '07 Apr 2024';

  $PROGRAM_NAME = 'HomeCare ' . "$PROGRAM_VERSION ($PROGRAM_DATE)";
  $PROGRAM_URL = 'http://k5n.us/wp/honecare/';
  $TROUBLE_URL = 'docs/not-supported';

  $settings = [];

  // Decide the source based on the HOMECARE_USE_ENV env variable
  $use_env = getenv('HOMECARE_USE_ENV');
  if ($use_env && strtolower($use_env) === "true") {
    // Load from environment variables
    foreach ($possible_settings as $key => $type) {
      $env_key = 'HOMECARE_' . strtoupper($key);
