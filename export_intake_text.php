<?php
/**
 * Plain-text export of intake history.
 *
 * Accepts `patient_id`, `start_date`, `end_date` query parameters —
 * identical surface to the CSV/FHIR/PDF exports so all formats are
 * interchangeable from the caller's perspective (see export.php). Role
 * gate + audit logging mirror the other exports for the same PHI reasons.
 */
...
if ($token) {
    ...
    if ($params !== null && $params['type'] === 'text' && (int) $params['patient_id'] === $patientId && $params['start_date'] === $startDate && $params['end_date'] === $endDate) {
        $authorized = true;
        $viaSigned = true;
    }
}
...
$text = (new TextIntakeExporter())->toText($rows, [
    'patient_name' => (string) $patient['name'],
    'period_label' => $periodLabel,
    'generated_at' => date('Y-m-d H:i'),
]);

audit_log('export.intake_text', 'patient', $patientId, [
